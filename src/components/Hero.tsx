...
export function Hero({ onSearch, onNavigate }: { onSearch: (term: string) => void, onNavigate?: () => void }) {
  return (
    <section className="relative w-full bg-slate-900 text-white overflow-hidden py-24 md:py-32 lg:py-40">
      ...
      <div className="absolute inset-0 bg-[url('/hero-bg.png')] bg-cover bg-[center_top] opacity-90 sm:opacity-100" />
      
      {/* 이미지가 자연스럽게 어울리도록 그라데이션 오버레이 추가: 모바일은 아래쪽, 데스크탑은 오른쪽으로 갈수록 어두워짐 */}
      <div className="absolute inset-0 bg-gradient-to-b from-slate-900/20 via-slate-900/90 to-slate-900 sm:bg-gradient-to-r sm:from-slate-900/0 sm:via-slate-900/70 sm:to-slate-900" />
