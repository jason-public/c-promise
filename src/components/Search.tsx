    ...
    <span>
      {parts.map((part, i) => 
        part.toLowerCase() === highlight.toLowerCase() ? 
          <mark key={i} className="bg-yellow-200 text-slate-900 rounded-sm px-0.5 font-semibold mx-px">{part}</mark> : part
      )}
    </span>
  );
};
    ...
    <section className="py-6 bg-white border-b border-slate-200 sticky top-20 z-40 shadow-sm">
      <div className="max-w-4xl mx-auto px-6 lg:px-8">
        <div className="relative group">
          ...
          <input
            type="text"
            className="block w-full pl-14 pr-12 py-4 border-2 border-slate-200 rounded-2xl leading-5 bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all sm:text-lg outline-none"
...
export function SearchResults({ searchTerm }: { searchTerm: string }) {
  ...
  if (!hasResults) {
    return (
      <section className="py-24 bg-slate-50 min-h-[50vh] flex flex-col items-center justify-center">
        <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-4">
          <SearchIcon className="w-8 h-8 text-slate-300" />
        </div>
        <p className="text-xl text-slate-500 font-medium">"{searchTerm}"에 대한 공약을 찾을 수 없습니다.</p>
  ...
  return (
    <section className="py-16 bg-slate-50 min-h-[50vh]">
      <div className="max-w-4xl mx-auto px-6 lg:px-8">
        <div className="mb-10 text-slate-600 bg-white inline-block px-4 py-2 rounded-lg border border-slate-200 shadow-sm">
          <span className="font-bold text-blue-600">"{searchTerm}"</span> 검색 결과
        </div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {results.core.map((pledge, idx) => (
                  <div key={idx} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 hover:border-blue-200 transition-colors">
